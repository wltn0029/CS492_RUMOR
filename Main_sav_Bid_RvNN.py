...
import os
import sav_Bid_RvNN as Bid_RvNN
import TD_RvNN
import BU_RvNN
...
def get_model_path(last_epoch, model_dir):
    return model_dir + '/model_' + str(last_epoch) + '.pkl'

model_dir = './Bid_models'
if not os.path.isdir(model_dir):
    os.mkdir(model_dir)
if len(sys.argv) >1:
    last_epoch = int(sys.argv[1])
    load_model_path = get_model_path(last_epoch, model_dir)
else:
    last_epoch = 0
...
if load_model_path is not None and os.path.exists(load_model_path):
    print('Load model : %s'%load_model_path)
...
losses_5, losses = [], []
num_examples_seen = 0
for epoch in range(last_epoch + 1, Nepoch):
    ## one SGD 
    ...
    ## cal loss & evaluate
    if epoch % 5 == 0:
       save_path = get_model_path(epoch, model_dir)
